...
const MovieDetailPage = () => {
  ...
  // tracks watchlist state for related movie cards
      ...
      try {
        ...
        if (token) {
          try {
            ...
            // fetch full watchlist so related MovieCards show correct Add/In List state
          ...
          // Log search_click — Contribution 2: passive browsing intent signal
          try {
            await api.post("/activity/log", {
          ...
          } catch (_) {}  // silent — never block page load
        }

        const uniqueRelatedMovies = recRes.data.filter(
          (movie, index, self) =>
            index === self.findIndex((m) => m.id === movie.id),
        );

        setRelatedMovies(uniqueRelatedMovies);
      } catch (err) {
  ...
  const handleWatchMainTrailerClick = async () => {
    try {
      ...
      // FIX #5: log trailer watch activity
  ...
  const handleWatchRelatedTrailerClick = async (movie) => {
    const relatedMovieId = movie?.id ?? movie;  // handles both full object and raw id
    if (!relatedMovieId) {
  ...
  const handleWatchlistToggle = async (movieToToggle) => {
    ...
    try {
      // FIX #4: log watchlist activity so Dashboard feed updates in real-time
