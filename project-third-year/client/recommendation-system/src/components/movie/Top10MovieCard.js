...
if (typeof document !== "undefined" && !document.getElementById("top10-styles")) {
  ...
  tag.textContent = `
    ...
    .t10-watchlist-btn:not(.is-in-watchlist):hover {
    ...
    /* Green saved state */
    .t10-watchlist-btn.is-in-watchlist {
    ...
    .t10-watchlist-btn.is-in-watchlist .icon-close {
      display: none;
    }
    .t10-watchlist-btn.is-in-watchlist .icon-check {
      display: block;
    }

    /* Red remove state on hover */
    .t10-watchlist-btn.is-in-watchlist:hover {
      background: rgba(239, 68, 68, 0.95);
      border-color: rgba(239, 68, 68, 0.4);
      box-shadow: 0 4px 12px rgba(239, 68, 68, 0.4);
    }
    .t10-watchlist-btn.is-in-watchlist:hover .icon-check {
      display: none;
    }
    .t10-watchlist-btn.is-in-watchlist:hover .icon-close {
      display: block;
    }
  `;
...
const Top10MovieCard = ({
  ...
  return (
    <div className="t10-card" onClick={() => navigate(`/movie/${toMovieSlug(movie)}`)}>
      ...
      {onWatchlistClick && (
        <button
          className={`t10-watchlist-btn${isInWatchlist ? " is-in-watchlist" : ""}`}
          onClick={(e) => {
            e.stopPropagation();
