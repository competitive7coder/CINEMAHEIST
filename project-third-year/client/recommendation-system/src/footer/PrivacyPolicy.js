...
const sections = [
  ...
  },
  {
    ...
    content: [
      { heading: "Storage", text: "Your data is stored in MongoDB Atlas with encryption at rest. Passwords are hashed using bcrypt and never stored in plain text." },
      { heading: "JWT Authentication", text: "We use JSON Web Tokens (JWT) for session management. Tokens expire automatically and are never stored on our servers." },
      { heading: "Retention", text: "We retain your account data as long as your account is active. You may request deletion at any time by contacting us." },
    ]
  },
  {
    num: "6",
        ...
        <div key={s.num} className="legal-section">
          ...
          {s.content.map((item, i) => (
            <div key={i} className="legal-item">
              <div className="legal-item-heading">{item.heading}</div>
              <div className="legal-item-text">{item.text}</div>
            </div>
          ))}
        </div>
      ))}
